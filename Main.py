...
"""
Define sources and pipelines to run and serve data from here. For example:

PIPELINES = [ # a list of mappings from sources to pipelines using images from each source
    (WebcamSource("/dev/cam/intake"), [BallPipeline(), BottomGoalPipeline()]) # a tuple (source, [pipelines that will use images from that source])
    , (WebcamSource("dev/cam/shooter"), [TurretPipeline()])
]
"""
PIPELINES = [
