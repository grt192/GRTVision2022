...
def draw_center_point(frame, x, y):
    cv2.line(frame, (x - config.center_line_length, y), (x + config.center_line_length, y), config.color,
             config.line_thickness)
    cv2.line(frame, (x, y - config.center_line_length), (x, y + config.center_line_length), config.color,
             config.line_thickness)


def draw_text(frame, x, y, text):
    temp = cv2.putText(frame, text, (x + config.text_offset, y - config.text_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                       config.color, config.line_thickness)
    frame = temp
